...
public class Bank extends Thread {
	...
	public void run() {
		...
		this.sampler = new Sampler(this, this.samplingRate);
		for(int i=1;i<=this.tellerCount;i++) {
			Teller t = new Teller(i, this.tellerIdleMean, this.tellerIdleVar, this);
			this.tellers.add(t);
			t.start();
		}
		...
		this.sampler.start();
		int index = 1;
		while(this.clock.isWorking()) {
				int serviceTime = gaussian(custServeTimeMean, custServeTimeVar);
				Customer c = new Customer(index++, this, serviceTime);
				synchronized (queue) {
					this.queue.offer(c);
					queue.notify();
				}			
				try {
					Bank.sleep(gaussian(custArrivalMean, custArrivalVar)*Bank.TIME_SIMULATION_FACTOR);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}	
		}
		if(!this.getCustomersQueue().isEmpty()) {
			synchronized (this) {
				try {
					this.wait();
				} catch (InterruptedException e) {
